...
namespace TerraStory.NPCs.Ludibrium
{
	...
	public class KingBloctopus : ModNPC
	{
		public override void SetStaticDefaults() 
		{
			DisplayName.SetDefault("King Bloctopus"); // Automatic from .lang files
		...
		public override void SetDefaults() 
		{
			...
			npc.value = 1f;
			npc.HitSound = mod.GetLegacySoundSlot(SoundType.NPCHit, "Sounds/NPCHit/KingBloctopusHit");
			npc.DeathSound = mod.GetLegacySoundSlot(SoundType.NPCKilled, "Sounds/NPCKilled/KingBloctopusDie");
		...
		public override void NPCLoot()
		{
			if (Main.rand.NextFloat() < .50f) // 50% chance
				Item.NewItem(npc.getRect(), ItemType<Items.BundleOfMesos>(), 2 - 5);
			if (Main.rand.NextFloat() < .50f) // 50% chance
				Item.NewItem(npc.getRect(), ItemType<Items.MapleLeaf>() , 2 - 5);
			if (Main.rand.NextFloat() < .50f) // 50% chance
