...
namespace TerraStory.NPCs.Bosses
{
	...
	public class OrangeMushmom : ModNPC
	{

		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Mushmom");
		...
		public override void SetDefaults()
		{
			...
			bossBag = mod.ItemType("OrangeMushmomTreasureBag");
		}
		...
		{
			World.downedOrangeMushmom = true;
			if (Main.expertMode)
			{
				npc.DropBossBags();
			}
			else
			{
				...
				Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("MushmomSpore"), Main.rand.Next(3, 10));
				...
				if (Main.rand.NextFloat() < .10f) // 10% chance
					Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("MushmomTrophy"), 1);
				...
				if (choice == 0)
					Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("MushMush"), 1);
				if (choice == 1)
				...
				if (choice == 3)
					Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("Subi"), Main.rand.Next(10, 100));
				if (choice == 4)
					Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("CopperArrow"), Main.rand.Next(10, 100));
		...
		public override void HitEffect(int hitDirection, double damage)
		{
			if (npc.life <= 0) /// this make so when npc die he will drop this
			{
				...
				Main.NewText("Mushmom : I really wanted to play some more...", Color.Red);
			}
		}
		public override void PostAI()
		{
			float distance = npc.Distance(Main.player[npc.target].Center);
			if (distance <= 300 && npc.velocity.Y == 0 && Main.player[npc.target].velocity.Y == 0)
			{
				Main.player[npc.target].AddBuff(BuffType<MushmomA>(), 2, true);
			}
			if (npc.velocity.Y == 0)
			{
		...
		public override void AI()
		{
			...
			if (this.npc.ai[1] == 5f)
			{
				...
				if (!flag101)
				{
					...
					{
						int num738 = Dust.NewDust(npc.position + Vector2.UnitX * -20f, npc.width + 40, npc.height, 4, npc.velocity.X, npc.velocity.Y, ModContent.DustType<TransparentDust>(), new Color(253, 245, 230, 80), 2f);
			...
			else if (this.npc.ai[1] == 6f)
			{
				...
				{
					int num740 = Dust.NewDust(npc.position + Vector2.UnitX * -20f, npc.width + 40, npc.height, 4, npc.velocity.X, npc.velocity.Y, ModContent.DustType<TransparentDust>(), new Color(253, 245, 230, 80), 2f);
			...
			for (int num746 = 0; num746 < num745; num746++)
			{
				...
				int MushHelpers = NPCType<OrangeMushroom>();
				if (Main.expertMode && Main.rand.Next(4) == 0)
				{
					MushHelpers = NPCType<OrangeMushroom>();
				}
