...
namespace TerraStory.Content.Recipes
{
    public static class RecipeHelper
    {
                ...
                for (int l = 0; l < 40; l++)
                {
                    ...
                    if (item3.stack > requiredAmount)
                    {
                        ...
                        if (Main.netMode == NetmodeID.MultiplayerClient && Main.player[Main.myPlayer].chest >= 0)
                            NetMessage.SendData(MessageID.SyncChestItem, -1, -1, null, Main.player[Main.myPlayer].chest, l);
                        requiredAmount = 0;
                    }
                    else
                    {
        ...
        public static void FindRecipes()
        {
            ...
            bool flag = Main.guideItem.type > ItemID.None && Main.guideItem.stack > 0 && Main.guideItem.Name != "";
            if (flag)
            {
                for (int j = 0; j < Terraria.Recipe.maxRecipes; j++)
                {
                    if (Main.recipe[j].createItem.type == ItemID.None)
                        break;
                    int num3 = 0;
                    while (num3 < Terraria.Recipe.maxRequirements && Main.recipe[j].requiredItem[num3].type != ItemID.None)
                    {
            ...
            else
            {
                ...
                if (Main.player[Main.myPlayer].chest != -1)
                {
                    if (Main.player[Main.myPlayer].chest > -1)
                        ...
                        switch (Main.player[Main.myPlayer].chest)
                        {
                            //Inventory Bank Main 1
                            case -2:
                                array = Main.player[Main.myPlayer].bank.item;
                                break;
                            //Inventory Bank Page 2
                            case -3:
                                array = Main.player[Main.myPlayer].bank2.item;
                                break;
                            //Inventory Bank Page 3
                            case -4:
                                array = Main.player[Main.myPlayer].bank3.item;
                                break;
                        }
