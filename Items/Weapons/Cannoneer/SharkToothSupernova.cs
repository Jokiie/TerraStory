...
{
	public class SharkToothSupernova : CannonDamageItem
	{
		public override void SetStaticDefaults()
		{
			Tooltip.SetDefault("Shoot a super slow big huge gigantic cannonball!");
		}
		...
		public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
		{
			...
			if (type == ProjectileType<NoviceBombProj>() || type == ProjectileType<IntermediateBombProj>() || type == ProjectileType<ExpertBombProj>()) 
			{
				...
				{
					Projectile.NewProjectile(position.X + 300, position.Y, speedX, speedY, type, damage, knockBack, player.whoAmI, 0f, 0f);
				}
				else
					Projectile.NewProjectile(position.X - 300, position.Y, speedX, speedY, type, damage, knockBack, player.whoAmI, 0f, 0f);
			}
