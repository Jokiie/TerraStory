...
namespace TerraStory.Projectiles.Minions.MapleGreenSlime
{
    public class MapleGreenSlime : ModProjectile
    {
        ...
        public override void AI()
        {
			...
			player.ClearBuff(BuffType<MapleGreenSlimeBuff>());
			}
			if (player.HasBuff(BuffType<MapleGreenSlimeBuff>())) {
				projectile.timeLeft = 2;
			}
            TerraStoryPlayer modPlayer = player.GetModPlayer<TerraStoryPlayer>();
            if (player.dead)
            {
                modPlayer.MapleGreenSlime = false;
            }
            if (modPlayer.MapleGreenSlime)
            {
                projectile.timeLeft = 2;
            }
            #endregion
