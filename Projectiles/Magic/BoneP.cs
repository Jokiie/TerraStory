...
namespace TerraStory.Projectiles.Magic
{
	public class BoneP : ModProjectile
	{
		...
		{
			DisplayName.SetDefault("Bones projectile");
		}
		...
		public override void AI()
		{
			...
			int num = Projectile.NewProjectile(dustPosition, projectile.velocity, ProjectileType<MiniBones>(), 5, 0, default, 2f);
		...
		public override void Kill(int timeLeft)
		{
			...
			ProjectileAnimations.Explode(projectile.whoAmI, 120, 120,
				delegate
				{
					for (int i = 0; i < 8; i++)
					{
						int num = Projectile.NewProjectile(projectile.position, projectile.velocity, ProjectileType<MiniBones>(), 5, 0, default, 2f);
						Main.projectile[num].position.X += Main.rand.Next(-50, 51) * .05f - 1.5f;
						Main.projectile[num].position.Y += Main.rand.Next(-50, 51) * .05f - 1.5f;
