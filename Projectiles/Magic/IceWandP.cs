...
{
	public class IceWandP : ModProjectile
	{
		...
		public override void AI()
		{
			...
			int num = Projectile.NewProjectile(dustPosition, projectile.velocity, ProjectileType<MiniIceWandP>(), 3, 0, default, 2f);
		...
		public override void Kill(int timeLeft)
		{
			...
			SoundManager.PlaySound(Sounds.Drip, (int)projectile.position.X, (int)projectile.position.Y, 14);
			ProjectileAnimations.Explode(projectile.whoAmI, 120, 120,
				delegate
				{
					for (int i = 0; i < 5; i++)
					{
						int num = Projectile.NewProjectile(projectile.position, projectile.velocity, ProjectileType<MiniIceWandP>(), 3, 0, default, 2f);
						Main.projectile[num].position.X += Main.rand.Next(-50, 51) * .05f - 1.5f;
						Main.projectile[num].position.Y += Main.rand.Next(-50, 51) * .05f - 1.5f;
