...
{
	public class PlatinumArrowP : ModProjectile
	{
		...
		public override void Kill(int timeLeft)
		{
			...
			{
				int item = Main.rand.NextBool(5) ? Item.NewItem(projectile.getRect(), mod.ItemType("PlatinumArrow")) : 0;
			}
			Main.PlaySound(SoundID.Item, (int)projectile.position.X, (int)projectile.position.Y, 64);
			for (int num158 = 0; num158 < 20; num158++)
			{
				int num159 = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, 91, projectile.velocity.X * 0.1f, projectile.velocity.Y * 0.1f, 0, default(Color), 0.5f);
		...
		public override void AI()
		{
			...
			if (projectile.localAI[0] > 3f)
			{
				...
				{
					int num92 = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y + 2f), projectile.width, projectile.height, 91, projectile.velocity.X * 0.2f, projectile.velocity.Y * 0.1f, 100, default(Color), 0.4f);
