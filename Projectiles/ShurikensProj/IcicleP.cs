...
namespace TerraStory.Projectiles.ShurikensProj
{
	public class IcicleP : ModProjectile
	{
		...
		{
			Main.PlaySound(SoundID.Item27, (int)projectile.position.X, (int)projectile.position.Y); // Play a death sound
			...
			if (projectile.owner == Main.myPlayer)
			{
				...
				Main.rand.NextBool(18)
					? Item.NewItem(projectile.getRect(), ModContent.ItemType<Icicle>())
					: 0;
		...
		private const int MAX_STICKY_projectileS = 6; // This is the max. amount of projectiles being able to attach
		private readonly Point[] _stickingprojectiles = new Point[MAX_STICKY_projectileS]; // The point array holding for sticking projectiles
		...
		private void UpdateStickyProjectiles(NPC target)
		{
			...
			for (int i = 0; i < Main.maxProjectiles; i++) 
			{
				...
				if (i != projectile.whoAmI 
					...
					&& currentProjectile.modProjectile is IcicleP IcicleProjectile
					&& IcicleProjectile.IsStickingToTarget 
					&& IcicleProjectile.TargetWhoAmI == target.whoAmI)
				{

					_stickingprojectiles[currentprojectileIndex++] = new Point(i, currentProjectile.timeLeft); 
					if (currentprojectileIndex >= _stickingprojectiles.Length)  
						break;
				}
			}

			
			if (currentprojectileIndex >= MAX_STICKY_projectileS)
			{
				int oldprojectileIndex = 0;
				
				for (int i = 1; i < MAX_STICKY_projectileS; i++)
				{
					
					if (_stickingprojectiles[i].Y < _stickingprojectiles[oldprojectileIndex].Y)
					{
						oldprojectileIndex = i; 
					}
				}
				Main.projectile[_stickingprojectiles[oldprojectileIndex].X].Kill();
			}
		}
		...
		public override void AI()
		{
			...
			if (projectile.frameCounter >= frameSpeed)
			{
				...
				else if (projectile.velocity.X == 0 || projectile.velocity.Y == 0)
                {
					projectile.frame = 3;
                }
			}
		...
		private void NormalAI()
		{
			...
			{
				Dust dust = Dust.NewDustDirect(projectile.position, projectile.height, projectile.width, 16,
		...
		private void StickyAI()
		{
			...
			const int aiFactor = 15; // Change this factor to change the 'lifetime' of this sticking projectile
			projectile.localAI[0] += 1f;

			// Every 30 ticks, the projectile will perform a hit effect
