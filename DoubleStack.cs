...
namespace TerraStory
{
    public class DoubleStack<T1>
	{
		...
		public void PushFront(T1 front)
		{
			...
			_start--;
			T1[] array2 = _segmentList[_start / _segmentSize];
		...
		public T1 PopFront()
		{
			...
			T1[] array = _segmentList[_start / _segmentSize];
			int num = _start % _segmentSize;
			T1 result = array[num];
			...
			{
				T1[] array2 = _segmentList[0];
				for (int i = 0; i < _segmentCount - 1; i++)
				{
					_segmentList[i] = _segmentList[i + 1];
				}
				_segmentList[_segmentCount - 1] = array2;
				_start -= _segmentSize;
				_end -= _segmentSize;
			}
			if (_size == 0)
		...
		public void PushBack(T1 back)
		{
			...
			T1[] array2 = _segmentList[_end / _segmentSize];
		...
		public T1 PopBack()
		{
			...
			T1[] array = _segmentList[_end / _segmentSize];
			int num = _end % _segmentSize;
			T1 result = array[num];
			array[num] = default(T1);
			_end--;
